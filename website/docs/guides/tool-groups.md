...
description: Per-session visibility for the 51 tools. Activate vfx, animation, ui, testing, etc. only when you need them.
---

# Tool Groups

MCP for Unity ships 51 tools, but exposing all of them to the LLM at once balloons the prompt and dilutes routing decisions. So tools are sorted into **groups**, and only `core` is enabled by default.
...
2. **Routing clarity**: when the LLM picks between 51 tools versus the 31 core tools, the wrong-tool rate drops measurably.
